/*
Copy from https://github.com/lmmx/gdocs2md-html with minor modifications.
...
function processParagraph(index, element, inSrc, imageCounter, listCounters, image_path) {
  ...
  for (var i = 0; i < element.getNumChildren(); i++) {
    ...
    if (t === DocumentApp.ElementType.TABLE_ROW) {
      ...
      imageCounter++;
      if (!return_string || force_save_images) {
        textElements.push('![](' + image_path.replace(/ /g, "%20") + '/' + name + ')');
      } else {
        textElements.push('![](' + imglink.replace(/ /g, "%20") + ')');
      }
      //result.images.push( {
      //  "bytes": blob.getBytes(), 
      //  "type": contentType, 
...
//    else {
//      throw "File"+image_path+"Paragraph "+index+" of type "+element.getType()+" has an unsupported child: "
